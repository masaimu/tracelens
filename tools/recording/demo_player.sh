...
case "${1:-}" in
  ...
  progress_bar)   banner "S2" "进度条 · progress.md" "0:53"; show_n design/progress.md 18 26 "当前快照 · 基线 bb6b81b · 83%";;
  #  --- S2b 短 prompt 闭环 ---
  ...
  dup_fixture)    banner "S3" "返工 · 重复 spanId 样例" "1:48"; box "fixture 里同一个 spanId 出现两次，分属不同 name / 时间区间"; awk '{ if ($0 ~ /spanId/) {c="\033[1;33m"} else {c=""}; printf "%s%4d │ %s\033[0m\n",c,NR,$0 }' tests/fixtures/otlp-duplicate-span.json;;
  prompt_dup)     banner "S3" "触发返工的那条 prompt" "1:57"; printf '  \033[1;32m▶ prompt:\033[0m 给 critical-path 加 span 汇总\n\n  \033[1;31mAI 第一版：\033[0m 按 spanId 去重 → 把两个不同实例合并成一个 → self time 算错\n\n  \033[1;32m返工约束：\033[0m 按「内部 span 实例聚合，不去重」，并补 fixture + 测试钉死\n';;
  test_code)      banner "S3" "钉死返工的测试 · critical_path.rs" "2:06"; show_n src/analysis/critical_path.rs 515 545 "duplicate_span_ids_are_not_merged_in_totals";;
  run_test)       banner "S3" "验收 · 跑测试" "2:14"; cargo test --bin tracelens duplicate_span_ids_are_not_merged_in_totals 2>&1 | tail -8;;
  #  --- S4 验收 ---
  ...
  #  --- S6 收尾 ---
  progress_bar_end) banner "S6" "收尾 · 进度条" "4:00"; show_n design/progress.md 18 26 "83%";;
  gitlog)         banner "S6" "提交历史（每个提交 = 一个迭代）" "4:07"; git log --oneline | head -24 | cat;;
