...
STORYBOARD = [
    ...
    (83,  "iter_report",    "这一期最终交付：新增了 timeline 命令、补上测试、写了实施报告——都是文档里写明的事实，不是我嘴上说说的。"),
    (95,  "HOLD",            "完整上下文早躺文档里被追踪，我不必每次重新交代。"),

    (103, "segintro_s3",    "AI 一定会翻车，我靠验收把它钉回去。"),
    (108, "release_fail",   "这次返工来自真实发版：我把 v0.1.1 标签推上去，GitHub Actions 在发布这一步直接红了。"),
    (117, "release_why",    "报错是 delete-a-release-asset 404——发布脚本删旧产物时，解不到那条已经失效的 asset id。"),
    (126, "release_fix",    "我没让它只补表面：先找根因，再把整步换成幂等的 gh release create——发布前先清掉同名残留 release，删旧这一步再也回不到 404。"),
    (134, "release_green",  "换个方式重推 v0.1.1，这次 run 绿了，四个平台八份产物齐——翻车不靠眼盯，靠重新发一遍把它验出来。"),
    (143, "HOLD",            "AI 最爱在看着对、其实漏了边的地方偷懒；我靠一次红到绿的真实发布把它焊死，这比补一条单测更狠。"),
