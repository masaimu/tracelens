...
STORYBOARD = [
    ...
    (10,  "HOLD",            "但今天想展示的不是它有多少功能，而是我怎么跟 AI 协作，把它逼成现在这样。"),
    (20,  "HOLD",            "我有一套自己摸出来的方法。"),

    (22,  "segintro_s2",    "先说方法论前半段：我先把需求拆成文档，再让 AI 照着文档做。"),
    (27,  "design_list",     "看这个 design 目录——需求、里程碑、进度条、传播规约、十六份迭代契约，全是 AI 和我一起维护的。"),
    (35,  "intro_abil",      "第一份是需求文档，把要覆盖的能力一条条列清楚。"),
    (41,  "milestones_table","再设里程碑，每条带权重和完成度，按重要性算贡献。"),
    (47,  "milestones_nogo", "还明确写了哪些不做，范围不被顺手扩大。"),
    (53,  "progress_bar",   "进度条量化到 83%，距离目标还差多少一目了然。"),

    (59,  "segintro_s2b",   "方法论的关键：我每次用一句话 prompt，就能调起一整期迭代。"),
    (64,  "prompt_short",   "所以开新迭代时，我的 prompt 短得离谱：按第 15 期文档执行，做完给报告。"),
    (73,  "iter_doc",        "但这一句调起的，是完整的 iteration-15 文档——本期目标、验收标准、产出物，全在里面写清。"),
    (83,  "iter_report",    "这一期最终交付：新增 timeline 命令、补了测试、给了实施报告——全是文档里的事实，不是我嘴说的。"),
    (95,  "HOLD",            "完整上下文早躺文档里被追踪，我不必每次重新交代。"),

    (103, "segintro_s3",    "AI 一定会翻车，我靠验收把它钉回去。"),
    (108, "dup_fixture",    "就拿这次返工说：同一条 trace 里偏有两个 span 共用一个 id、还分属不同服务。"),
    (117, "prompt_dup",     "我让它给关键路径加 span 汇总，它第一版顺手按 spanId 去重，self time 直接算错。"),
    (126, "test_code",       "我没让它蒙过去——加一条测试，断言这两个实例不许合并。"),
    (134, "run_test",        "现在这条测试是绿的，一个归 cart，一个归 payment。"),
    (143, "HOLD",            "AI 最爱在「看着对、其实漏了一块」的地方偷懒，我用一条数据加一条测试把它焊死。"),

    (148, "segintro_s4",    "验收怎么可复？把约束写进 AGENTS.md，不靠记。"),
    ...
    (240, "progress_bar_end","16 个迭代、21 个提交、83%。"),
    (247, "gitlog",         "每一步都有文档、有验收、有返工。"),
    (255, "endcard",        "把 AI 用好不靠口才，是把上下文和约束沉淀成轨道——让它跑、我来验。"),
]
